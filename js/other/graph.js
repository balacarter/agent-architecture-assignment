 ...
 * 7-30-2013 Miguel Vazquez  Changed variable names to be more generic so that if scenario changes same variables can be used
 * 6-06-2016 James Yen       Changed "hover" to display x,y coordinates, allowed for adding obstacles via x,y coordinates.
 * 6-07-2016 James Yen       Adding an obstacle causes the new obstacles coordinates to appear in the list on the page, as opposed to showing the number of the tile.
 * 6-08-2016 James Yen       All obstacle clicking and adding, and removing functions should be updated.
...
Graph.prototype.createEntireGraph = function(index){
    ...
    var tempArray = listOfObstacles.match(/\(.*?\)/g);
    var arrayOfObstacles = new Array();
  
    for (var i = 0; i < tempArray.length; i++) {
        var coordinate = tempArray[i];
        var coordinate = coordinate.substring(1, coordinate.length - 1);
        var xy = coordinate.split(",");
        var x = parseInt(xy[0]);
        var y = parseInt(xy[1]) * parseInt($('#width option:selected').val());
        var place = x + y;
        arrayOfObstacles.push(place); 
    }
...
Graph.prototype.enableVertexHoverText = function () {
    ...
    $(htmlContainer + " > .vertex").hover(
        function () {
            ...
            $("#" + popupId).css({"left": (x + vertexDiameter + 10) + "px", "top" : y + "px"});
            var x = parseInt($(this).attr("name"));
            x = x % parseInt($('#width option:selected').val());
            var y = parseInt($(this).attr("name"));
            y = Math.floor(y / parseInt($('#width option:selected').val()));
            $("#" + popupId).html("("  + x + "," + y + ")");
        },
        function () {
    ...
    $(htmlContainer + " > .vertex").dblclick(
        function () {
            ...
            if(document.getElementById("obstacle").checked && index !== -1) {
                ...
                document.getElementById("obstacleList").value = tempArray;
            }
            arrayOfObstacles = [];
        }
    );
    ...
    $(htmlContainer + " > .vertex").contextmenu(
        function () {
            ...
            if(valueChange == 0 && document.getElementById("obstacle").checked && !graphData.vertices[tileToValue].isObstacle){
                ...
                var width = parseInt($('#width option:selected').val());
                var y = Math.floor(parseInt(tileToValue)/width);
                var x = parseInt(tileToValue)%width;
                var stringVal = "(" + x + "," + y + ")";
                document.getElementById("obstacleList").value = arrayList + stringVal;
            } else {
