 ...
 * 6-22-2016 James Yen       Entering starting points for the lizard agents via (x,y) notation.
...
$(function () {
    var iterationInterval,                      // interval id for running the solution animation
		...
		moveSpriteTo = function (newTop, newLeft, number) {
            ...
            $("#sprite" + number).attr('display','inline');
		},
        ...
        prepareRoots = function(graph){
        	...
        	for(var i = 0 ; i < numberOfRoots ; i++){
                ...
                var coordinates = document.getElementById("start" + i).value;
                var xy = coordinates.split(",");
                var re = /[0-9]*$/;

                var match = xy[0].match(re);
        ...
        moveSprites = function(graph){
        	...
        	for(var i = 0 ; i < numberOfRoots ; i++){
        		moveSpriteTo(graph.vertices[graph.roots[i]].y + 3,graph.vertices[graph.roots[i]].x + 1, i);
        	} 
        }

        /**
         * Hides Html Elements that represent agents not being used
         * @method basic_search#hideUnusedSprites
         * @param {graph} graph this is the HTML Element that will be acted upon
         */
        hideUnusedSprites = function(graph){
            var numberOfRoots = $('#quantity option:selected').val();
            
            for(var i = 3 ; i >= numberOfRoots ; i--){
                $("#sprite" + i).css('display', 'none');
            } 
        }  
